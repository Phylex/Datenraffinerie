...
class ControlAdapter:
    ...
    def __init__(self, hostname, port, config):
        """
        Initialize the data structure on the control computer (the one coordinating everything)
        and connect to the system component. Do not load any configuraion yet
        """
    ...
    def reset(self):
        """
        reset the connection with the system component, may not reset the state of the component
        """
        self.socket.close()
        context = zmq.Context()
    ...
    def configure(self, config):
        """
        send the configuration to the corresponding system component and wait for the
...
class TargetAdapter(ControlAdapter):
    ...
    def read_config(self, parameter: dict):
        ...
        Returns:
            the dict containing the requested parameter(s) passed in the parameter
            argument set to the value read from the ROC
        """
